...
def get_spark():
    return (
        ...
        .appName("ETL_StarSchema_to_Valkey")
        .getOrCreate()
    )
...
def write_to_valkey(rows, key_prefix, id_field, rank_field=None, batch_size=500):
    r = get_valkey()
    pipe = r.pipeline()

    existing = r.keys(f"{key_prefix}:*")
    if existing:
        pipe.delete(*existing)
    if rank_field:
        pipe.delete(f"{key_prefix}:_index")

    for row in rows:
        key = f"{key_prefix}:{row[id_field]}"
        data = {k: str(v) for k, v in row.items()}
        pipe.hset(key, mapping=data)
        if rank_field and row.get(rank_field) is not None:
            pipe.zadd(f"{key_prefix}:_index", {key: float(row[rank_field])})

    pipe.execute()
    r.close()
    print(f"  -> {key_prefix}: {len(rows)} entries written to Valkey")
...
def main():
    ...
    write_to_valkey(df_to_dicts(report_product_sales), "report:product_sales", "product_id", "rank_by_sales")

    print("Creating report:customer_sales...")
    ...
    write_to_valkey(df_to_dicts(report_customer_sales), "report:customer_sales", "customer_id", "rank_by_spend")

    print("Creating report:time_sales...")
    ...
    write_to_valkey(df_to_dicts(report_time_sales), "report:time_sales", "year_month", "sale_month")

    print("Creating report:store_sales...")
    ...
    write_to_valkey(df_to_dicts(report_store_sales), "report:store_sales", "store_id", "rank_by_revenue")

    print("Creating report:supplier_sales...")
    ...
    write_to_valkey(df_to_dicts(report_supplier_sales), "report:supplier_sales", "supplier_id", "rank_by_revenue")

    print("Creating report:product_quality...")
    ...
    write_to_valkey(df_to_dicts(report_product_quality), "report:product_quality", "product_id", "rank_by_rating_desc")
